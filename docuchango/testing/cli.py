...
class CLIResult:
    ...
    def assert_contains(self, text: str) -> None:
        ...
        if text not in self.stdout:
            raise AssertionError(f"Expected stdout to contain '{text}'\nStdout: {self.stdout}")

    def assert_not_contains(self, text: str) -> None:
        """Assert that stdout does not contain text."""
        if text in self.stdout:
            raise AssertionError(f"Expected stdout to not contain '{text}'\nStdout: {self.stdout}")
...
class AGFCLIRunner:
    ...
    def _build_binary(self) -> None:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Failed to build AGF CLI:\n{result.stderr}")

        if not self.binary_path.exists():
            raise RuntimeError(f"Build succeeded but binary not found at {self.binary_path}")
    ...
    def fabric_create(self, name: str, description: str = "", region: str = "us-west-2", **kwargs) -> CLIResult:
        """Create a fabric."""
    ...
    def fabric_update(self, fabric_id: str, description: Optional[str] = None, **kwargs) -> CLIResult:
        """Update a fabric."""
    ...
    def operation_wait(self, operation_id: str, timeout: str = "5m", interval: str = "2s", **kwargs) -> CLIResult:
        """Wait for operation to complete."""
        return self.run("operation", "wait", operation_id, "--timeout", timeout, "--interval", interval, **kwargs)
